...
Updated to use PyTorch checkpoint inference results as ground truth
"""
...
def create_comparison_visualization(rgb, gt_depth, pytorch_fp32_pred, onnx_fp32_pred, npu_int8_pred, 
                                   pytorch_fp32_metrics, onnx_fp32_metrics, npu_int8_metrics,
                                   output_path, filename,
                                   min_depth=0.5, max_depth=15.0):
    """Create 3-way comparison visualization: PyTorch FP32 vs ONNX FP32 vs NPU INT8"""
    ...
    if pytorch_fp32_pred.shape != gt_depth.shape:
        scale_h = gt_depth.shape[0] / pytorch_fp32_pred.shape[0]
        scale_w = gt_depth.shape[1] / pytorch_fp32_pred.shape[1]
        pytorch_fp32_pred = zoom(pytorch_fp32_pred, (scale_h, scale_w), order=1)
    
    if onnx_fp32_pred.shape != gt_depth.shape:
        scale_h = gt_depth.shape[0] / onnx_fp32_pred.shape[0]
        scale_w = gt_depth.shape[1] / onnx_fp32_pred.shape[1]
        onnx_fp32_pred = zoom(onnx_fp32_pred, (scale_h, scale_w), order=1)
    ...
    # NPU INT8 Metrics (Orange background)
    y_start = 0.42
    rect_npu = mpatches.FancyBboxPatch((0.05, y_start - 0.28), 0.9, 0.26,
                                       boxstyle="round,pad=0.01",
    ...
    metrics_text_npu = (
        ...
        f"δ<1.25: {npu_int8_metrics['a1']:.4f} ({npu_int8_metrics['a1']*100:.1f}%)"
    )
    ...
    y_start = 0.09
    abs_rel_deg = (npu_int8_metrics['abs_rel'] - fp32_metrics['abs_rel']) / fp32_metrics['abs_rel'] * 100
    rmse_deg = (npu_int8_metrics['rmse'] - fp32_metrics['rmse']) / fp32_metrics['rmse'] * 100
    a1_deg = (npu_int8_metrics['a1'] - fp32_metrics['a1']) / fp32_metrics['a1'] * 100
    ...
    # === Row 2: FP32, NPU INT8, Absolute Error ===
    ...
    # 5. NPU INT8 Prediction
    ax5 = fig.add_subplot(gs[1, 1])
    
    npu_display = npu_int8_pred.copy()
    npu_display[~valid_mask] = min_depth
    
    im5 = ax5.imshow(npu_display, cmap='turbo_r', vmin=min_depth, vmax=max_depth)
    ax5.imshow(invalid_overlay)
    ax5.set_title('NPU INT8 Prediction', fontsize=16, fontweight='bold', pad=10)
    ax5.axis('off')
    
    cbar5 = plt.colorbar(im5, ax=ax5, fraction=0.046, pad=0.04)
    cbar5.set_label('Depth (m)', fontsize=12)
    
    # 6. Absolute Error (FP32 vs NPU INT8)
    ax6 = fig.add_subplot(gs[1, 2])
    
    abs_error = np.abs(fp32_pred - npu_int8_pred)
    ...
    ax6.set_title('Absolute Error (FP32 - NPU INT8)', fontsize=16, fontweight='bold', pad=10)
    ax6.axis('off')
    ...
    # Main title
    fig.suptitle(f'ONNX FP32 vs NPU INT8 Comparison - {filename}',
...
def main():
    print("=" * 80)
    print("🎨 ONNX FP32 vs NPU INT8 3-Way Comparison Visualization")
    print("=" * 80)
    
    # Paths
    fp32_output_dir = Path('outputs/onnx_fp32_direct_depth_inference')
    npu_int8_output_dir = Path('outputs/resnetsan_direct_depth_05_15_640x384')
    test_json_path = Path('/workspace/data/ncdb-cls-640x384/splits/combined_test.json')
    vis_output_dir = Path('outputs/onnx_fp32_vs_npu_int8_visualization')
    ...
    if not npu_int8_output_dir.exists():
        print(f"\n❌ Error: NPU INT8 results not found at {npu_int8_output_dir}")
        return
    ...
    for fp32_file in fp32_files:
        ...
        npu_file = npu_int8_output_dir / f'{filename}.npy'
        ...
        npu_int8_pred = np.load(npu_file)
        
        # Squeeze if needed (NPU outputs (1, H, W))
        if len(npu_int8_pred.shape) == 3:
            npu_int8_pred = npu_int8_pred[0]
        ...
        if fp32_pred.shape != gt_depth.shape:
            ...
            npu_int8_pred_resized = zoom(npu_int8_pred, (scale_h, scale_w), order=1)
        else:
            fp32_pred_resized = fp32_pred
            npu_int8_pred_resized = npu_int8_pred
        
        # Compute metrics
        fp32_metrics = compute_depth_metrics(gt_depth, fp32_pred_resized, min_depth, max_depth)
        npu_int8_metrics = compute_depth_metrics(gt_depth, npu_int8_pred_resized, min_depth, max_depth)
        
        if fp32_metrics is None or npu_int8_metrics is None:
            continue
        
        all_metrics.append({
            'filename': filename,
            'fp32_abs_rel': fp32_metrics['abs_rel'],
            'npu_int8_abs_rel': npu_int8_metrics['abs_rel'],
            'fp32_rmse': fp32_metrics['rmse'],
            'npu_int8_rmse': npu_int8_metrics['rmse'],
            'fp32_metrics': fp32_metrics,
            'npu_int8_metrics': npu_int8_metrics
        })
    ...
    for i, m in enumerate(best_5, 1):
        print(f"   {i}. {m['filename']} - FP32 abs_rel: {m['fp32_abs_rel']:.4f}, NPU INT8 abs_rel: {m['npu_int8_abs_rel']:.4f}")
    ...
    for i, metrics_dict in enumerate(selected_cases):
        ...
        npu_int8_pred = np.load(npu_int8_output_dir / f'{filename}.npy')
        ...
        create_comparison_visualization(
            rgb, gt_depth, fp32_pred, npu_int8_pred,
            metrics_dict['fp32_metrics'], metrics_dict['npu_int8_metrics'],
    ...
    avg_npu_int8_abs_rel = np.mean([m['npu_int8_abs_rel'] for m in all_metrics])
    avg_fp32_rmse = np.mean([m['fp32_rmse'] for m in all_metrics])
    avg_npu_int8_rmse = np.mean([m['npu_int8_rmse'] for m in all_metrics])
    ...
    print(f"\nNPU INT8:")
    print(f"  avg abs_rel: {avg_npu_int8_abs_rel:.4f}")
    print(f"  avg RMSE:    {avg_npu_int8_rmse:.3f}m")
    
    print(f"\nDegradation:")
    print(f"  abs_rel: {(avg_npu_int8_abs_rel - avg_fp32_abs_rel) / avg_fp32_abs_rel * 100:+.1f}%")
    print(f"  RMSE:    {(avg_npu_int8_rmse - avg_fp32_rmse) / avg_fp32_rmse * 100:+.1f}%")
